    ...
    return (
        <div className="section container vendor-wallet-container">
            ...
            <div className="balance-card" style={{
                ...
                <div className="balance-icon" style={{ fontSize: '3rem', background: 'rgba(255,255,255,0.2)', width: '80px', height: '80px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>💰</div>
                <div className="balance-content">
                    <h2 style={{ color: '#333333', fontSize: '1.1rem', margin: 0, fontWeight: 500 }}>
                        {t('vendorWallet.currentBalance') || 'Mevcut Bakiye'}
                    </h2>
                    <p style={{ color: '#000000', fontSize: '2.5rem', fontWeight: 700, margin: '5px 0 0 0' }}>
                        {vendorProfile?.credit_balance || 0} {t('vendorWallet.credits') || 'Kredi'}
                    </p>
                </div>
            </div>
                            ...
                            <tbody>
                                {filteredTransactions.map(txn => (
                                    <tr key={txn.id} className={txn.status === 'rejected' ? 'row-rejected' : ''}>
                                        ...
                                        <td style={{ textAlign: 'right', fontWeight: 'bold', color: (txn.credits_added || 0) > 0 ? '#2e7d32' : '#c62828' }}>
                                            {(txn.credits_added || 0) > 0 ? '+' : ''}{txn.credits_added || 0} {t('vendorWallet.credits')}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
